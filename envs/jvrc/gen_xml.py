...
# --- KEY CHANGE: Move OBSTACLE_CONFIG to the module's top level ---
# By defining it here, it becomes a global constant that can be imported by other scripts.
OBSTACLE_CONFIG = {
    'num': 5,
    'start_x': 3,
    'spacing_x': 4,
...
def builder(export_path, config):
    ...
    # The OBSTACLE_CONFIG dictionary is now defined at the top level,
    # but it is still used here to build the scene.

    print("DEBUG: Attempting to add obstacles...")
    for i in range(OBSTACLE_CONFIG['num']):
        # --- This now reads from the config dictionary ---
        pos_x = OBSTACLE_CONFIG['start_x'] + i * OBSTACLE_CONFIG['spacing_x']
        mjcf_model.worldbody.add('geom',
                                 name=f'obstacle_{i}',
                                 type='cylinder',
                                 size=f"{OBSTACLE_CONFIG['radius']} {OBSTACLE_CONFIG['height']}",  
                                 pos=f'{pos_x} 0 {OBSTACLE_CONFIG["height"]}', 
                                 rgba='0.8 0.2 0.2 1',
                                 contype='1',
                                 conaffinity='1')
