...
def _calc_foot_orient_reward(self):
    # foot orientation reward, penalize out-toeing
    root_yaw = tf3.euler.quat2euler(self._client.get_object_xquat_by_name(self._root_body_name, 'OBJ_BODY'))[2]
    lfoot_yaw = tf3.euler.quat2euler(self._client.get_object_xquat_by_name(self._lfoot_body_name, 'OBJ_BODY'))[2]
    rfoot_yaw = tf3.euler.quat2euler(self._client.get_object_xquat_by_name(self._rfoot_body_name, 'OBJ_BODY'))[2]
    
    # Calculate the difference, handling angle wrapping
    lfoot_error = abs(np.arctan2(np.sin(lfoot_yaw - root_yaw), np.cos(lfoot_yaw - root_yaw)))
    rfoot_error = abs(np.arctan2(np.sin(rfoot_yaw - root_yaw), np.cos(rfoot_yaw - root_yaw)))
    
    # Penalize deviation from the root yaw
    error = 10 * (lfoot_error + rfoot_error)
    return np.exp(-error)
