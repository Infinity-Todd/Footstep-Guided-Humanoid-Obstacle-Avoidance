...
from torch.cuda.amp import GradScaler, autocast  # Import autocast and GradScaler
...
class PPO:
    def __init__(self, env_fn, args):
        self.args = args  # Store args to access them in other methods
        ...
        # Ensure batch_size is divisible by num_procs
        assert self.batch_size % self.n_proc == 0, "Batch size must be divisible by number of processes."
        self.max_traj_len = self.batch_size // self.n_proc
        ...
        if args.continued:
            ...
            policy = torch.load(path_to_actor, weights_only=False, map_location=self.device)
            critic = torch.load(path_to_critic, weights_only=False, map_location=self.device)
            # policy action noise parameters are initialized from scratch and not loaded
            if args.learn_std:
                policy.stds = torch.nn.Parameter(args.std_dev * torch.ones(action_dim, device=self.device))
            else:
                policy.stds = args.std_dev * torch.ones(action_dim, device=self.device)
            ...
            with torch.no_grad():
                policy.obs_mean, policy.obs_std = map(lambda x: torch.tensor(x, device=self.device, dtype=torch.float32), (obs_mean, obs_std))
                critic.obs_mean = policy.obs_mean
                critic.obs_std = policy.obs_std

        base_policy = None
        if args.imitate:
            base_policy = torch.load(args.imitate, weights_only=False, map_location=self.device)
        ...
        # Add a GradScaler for mixed precision
    ...
    def sample(env_fn, policy, critic, gamma, lam, iteration_count, max_steps, max_traj_len, deterministic):
        ...
        """
        # Force CPU for sampling in ray workers to avoid MPS issues with multiprocessing
        ...
        while not memory_full:
            ...
            while not done and traj_len < max_traj_len:
                ...
                next_state, reward, done, _ = env.step(action.cpu().numpy().copy())
    ...
    # This method is NOT remote. It orchestrates remote workers.
    def sample_parallel(self, env_fn, policy, critic, deterministic=False):
        policy_ref = ray.put(policy.cpu())
        critic_ref = ray.put(critic.cpu())

        worker_args = (env_fn, policy_ref, critic_ref, self.gamma, self.lam, self.iteration_count, self.max_traj_len, self.max_traj_len, deterministic)
        workers = [self.sample.remote(*worker_args) for _ in range(self.n_proc)]
        
        return workers # Return the list of futures
    ...
    def update_actor_critic(self, obs_batch, action_batch, return_batch, advantage_batch, mask, mirror_observation=None, mirror_action=None):
        ...
        with autocast(enabled=(self.device.type != 'cpu')):
            ...
            if mirror_observation is not None and mirror_action is not None:
                ...
                mirror_loss = torch.zeros_like(actor_loss, device=self.device)

            # imitation loss
            if self.base_policy is not None:
                imitation_loss = (self.base_policy(obs_batch) - deterministic_actions).pow(2).mean()
            else:
                imitation_loss = torch.zeros_like(actor_loss, device=self.device)
        ...
        self.scaler.scale(total_actor_loss).backward(retain_graph=True)
        self.scaler.unscale_(self.actor_optimizer)  # Unscale before clipping
        torch.nn.utils.clip_grad_norm_(self.policy.parameters(), self.grad_clip)
        self.scaler.step(self.actor_optimizer)

        self.critic_optimizer.zero_grad()
        self.scaler.scale(critic_loss).backward(retain_graph=True)
        self.scaler.unscale_(self.critic_optimizer)  # Unscale before clipping
        ...
        return (
            ...
            mirror_loss,
            imitation_loss,
            torch.mean((torch.abs(ratio - 1) > self.clip).float()).item(),
        )
    ...
    def evaluate(self, env_fn, nets, itr, num_batches=5):
        ...
        for _ in range(num_batches):
            # sample_parallel returns a list of futures
            batch_futures = self.sample_parallel(env_fn, *nets.values(), deterministic=True)
            # wait for the results
            result = ray.get(batch_futures)
            
            # Aggregate results from all workers
            keys = result[0].keys()
            aggregated_data = {
                k: torch.cat([r[k] for r in result]).to(torch.float32).to(self.device) for k in keys
            }
            ...
            batch = Data(aggregated_data)
            eval_batches.append(batch)

        # Save the checkpoint with the iteration number (e.g., actor_99.pt)
        self.save(nets, self.save_path, "_" + repr(itr))

        # Also save the latest checkpoint as "_best", overwriting the previous one
        self.save(nets, self.save_path, "_best")

        # Log the evaluation reward for monitoring
        eval_ep_rewards = [float(i) for batch in eval_batches for i in batch.ep_rewards]
        avg_eval_ep_rewards = np.mean(eval_ep_rewards)
        print(f"Evaluation at iteration {itr}: Avg Reward = {avg_eval_ep_rewards:.3f}. Models saved as '{repr(itr)}' and 'best'.")

        return eval_batches

    def train(self, env_fn, n_itr):
        start_itr = 0
        if self.args.continued:
            filename = str(self.args.continued)
            # If continuing from a 'best' model, find the latest numbered model to determine the iteration
            if "_best.pt" in filename:
                log_dir = self.args.continued.parent
                # Find all numbered actor files, ensuring we work with Path objects
                numbered_files = [p for p in log_dir.glob('actor_*.pt') if "_best" not in str(p)]
                if numbered_files:
                    # Get the one with the highest iteration number using Path.stem
                    latest_file = max(numbered_files, key=lambda p: int(p.stem.split('_')[-1]))
                    start_itr = int(latest_file.stem.split('_')[-1]) + 1
                else:
                    # If only a 'best' model exists, start from iter 1
                    start_itr = 1 
                print(f"Continuing from best model, starting at iteration {start_itr}")
            else:
                # If continuing from a numbered file, parse the number
                try:
                    # Use Path object to safely get the stem
                    start_itr = int(self.args.continued.stem.split('_')[-1]) + 1
                except (ValueError, IndexError):
                    print(f"Warning: Could not parse iteration number from {filename}. Starting from iteration 0.")
                    start_itr = 0
        ...
        # Asynchronous sampling: kick off the first data collection task
        print("********** Kicking off initial data collection ************")
        batch_futures = self.sample_parallel(env_fn, self.policy, self.critic)
        ...
        for itr in range(start_itr, n_itr):
            ...
            # Kick off the next data collection task asynchronously
            next_batch_futures = self.sample_parallel(env_fn, self.policy, self.critic)

            sample_start_time = time.time()
            # Wait for the data from the previous iteration to be ready
            result = ray.get(batch_futures)
            # The new future becomes the one we wait for in the next iteration
            batch_futures = next_batch_futures
